...
`)
	...
	Convey("Tests for IsXxx()", t, func() {
		...
		fnames := []string{"IsTar", "IsGz", "Is7z", "IsZip", "IsTarGz", "IsTar7z", "IsZipOr7z", "IsZipOr7z", "IsExe", "IsMsi", "IsExeOrMsi", "IsExeOrMsi"}
		exts := []string{".tar", ".gz", ".7z", ".zip", ".tar.gz", ".tar.7z", ".zip", ".7z", ".exe", ".msi", ".exe", ".msi"}
